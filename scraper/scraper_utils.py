...
class YoutubeScraper(MediaScraper):
    ...
    thumbnail_template = 'http://img.youtube.com/vi/$video_id/maxresdefault.jpg'
    ...
    def media_object(self):
        ...
        if self.soup:
            video_url = (
                    self.soup.find('meta', dict(property='og:video')) or
                    self.soup.find('meta', dict(name='twitter:player'))
            )
            if video_url:
